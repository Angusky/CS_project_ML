...
KnnBayesOrderSemi::KnnBayesOrderSemi(vector<MyData> &X, vector<MyData> &XT, int k) {
	this->X = X;
	this->XT = XT;
	this->k = k;
	round_limit = 20;
...
void KnnBayesOrderSemi::setT(vector<MyData> &T) {
	//reset previous data
...
void KnnBayesOrderSemi::fillDismatrix() {
...
void KnnBayesOrderSemi::performTrans() {
...
void KnnBayesOrderSemi::getSortedMatrix(vector<vector<double>> &new_dis) {
	indexSortedMatrix(total_data, dis_matrixs[dis_matrixs.size() - 1], new_dis);
}
double KnnBayesOrderSemi::getScore() {
	vector<int> results;
	int train_data_size = X.size() + XT.size();
